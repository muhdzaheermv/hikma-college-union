...
const Content = ({event, setOpenRegModal,alreadyReg,setOpenStatusModal}) => {
 ...
 const router =  useRouter();
  ...
  const registerClicked = () =>{
    router.push('https://reg.ragam.co.in/workshops/'+event.id)
    return
    if(loginContext){
  ...
  return (
    <div    className={styles.contentContainer}>
        <div  className={styles.imageContainer}>
          {event?.posterImage&&<Image width={600} height={600} alt='poster' className={styles.show} src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${event?.posterImage.url}`}/> }
